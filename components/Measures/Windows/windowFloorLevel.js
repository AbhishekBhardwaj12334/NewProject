...
import { clearTempWindow, setOption, setStep } from '../../redux/measures';
...
const WindowFloor = ({ navigation }) => {
    ...
    const data = useSelector(state => state.windowOptionsData.windowOptionsArray.floorLevelData)
    // console.log('dsskk', JSON.stringify(details));

    const [windowFloorLevel, setWindowFloorLevel] = useState(data);
    ...
    // console.log('dshjdscdscd', localStep, selectedData, details?.selectedOptions[globalStep]?.optionId)
    ...
    const handleOptionClick = (optionsId, value) => {
        if (optionsId === windowFloorLevel.length) {
    ...
    const handleOtherSubmit = () => {
        ...
        setWindowFloorLevel(updatedData);
        setOtherText('');
