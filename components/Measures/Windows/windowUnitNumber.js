...
// import { updatewindowUnitNumber } from '../../redux/ext&intOption
import GlobalBottomSheet from '../globalRBSheet';
import ButtonComponent from './btnComponent';

const WindowUnitNumber = ({ navigation }) => {
    ...
    const data = useSelector(state => state.windowOptionsData.windowOptionsArray.windowUnitNumber)
    const [windowUnitNumber, setWindowUnitNumber] = useState(data);
    ...
    const handleOptionClick = (optionsId, value) => {
        if (optionsId === windowUnitNumber.length) {
    ...
    const handleOtherSubmit = () => {
        const otherOptionIndex = windowUnitNumber.findIndex(option => option.optionsId ? option.value === 'Other' : null);
        const otherOption = windowUnitNumber[otherOptionIndex];
        const newOptionId = otherOption.optionsId;
        const incrementedOtherOptionId = Math.max(...windowUnitNumber.map(option => option.optionsId)) + 1;
        ...
        const updatedData = [
            ...windowUnitNumber.slice(0, otherOptionIndex),
            newOption,
            updatedOtherOption,
            ...windowUnitNumber.slice(otherOptionIndex + 1)
        ];
        setWindowUnitNumber(updatedData);
        setOtherText('');
...
export default WindowUnitNumber;
