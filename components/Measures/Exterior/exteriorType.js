...
const TypeExterior = ({ navigation }) => {
    ...
    const data = useSelector(state => {
        if (templateName.templateId === '01') {
            return state.stormsecurityOptionsData.stormOptionArray.typeData
        } else if (templateName.templateId === '02') {
            return state.exteriorOptionsData.optionArray.typeData
        }
    })
    ...
    const handleOptionClick = (optionsId, value) => {
        if (templateName.templateId !== '01' && optionsId === typeData.length) {
